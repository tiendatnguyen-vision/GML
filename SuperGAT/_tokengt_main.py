...
def train_model(device, model, dataset_or_loader, criterion, optimizer, epoch, _args, lap_eigvec=None, rand_pe=None):
    model.train()
    try:
...
def _get_model_cls(model_name: str):
    if model_name == "TokenGT":
        return TokenGTNet
    elif model_name in ("GCN", "GIN", "GAT"):
        return _test_GNN
    elif model_name == "MLP":
        return _test_MLPNet
    else:
...
def run(args, gpu_id=None, return_model=False, return_time_series=False):
    ...
    if args.rand_pe:
        try:
            rand_pe = torch.load(f"binary-rand-{args.dataset_class}-{args.dataset_name}-{args.rand_pe_dim}.pt")
            ...
            torch.save(rand_pe, f"binary-rand-{args.dataset_class}-{args.dataset_name}-{args.rand_pe_dim}.pt")
    else:
        rand_pe = None

    if args.lap_pe:
        try:
            lap_eigvec = torch.load(f"lap-{args.dataset_class}-{args.dataset_name}-{args.lap_pe_k}.pt")
        except FileNotFoundError:
            assert len(train_d) == 1
            ...
            with torch.no_grad():
                ...
                # n =  18333
                # n =  18333
            torch.save(lap_eigvec, f"lap-{args.dataset_class}-{args.dataset_name}-{args.lap_pe_k}.pt")
    else:
        lap_eigvec = None
    ...
    for current_iter, epoch in enumerate(tqdm(range(args.start_epoch, args.start_epoch + args.epochs))):
        ...
        train_loss = train_model(running_device, net, train_d, loss_func, adam_optim,
